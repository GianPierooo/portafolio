 ...
 * Reactiva al mouse: parallax sutil vía lerp en useFrame (sin re-renders de React).
 */
function ConstellationNetwork() {
  ...
  // Parallax de mouse: puntero normalizado [-1, 1] y offset suavizado.
  // Todo en refs — cero re-renders de React, cero allocations por frame.
  ...
  useFrame((state, delta) => {
    if (groupRef.current) {
      // Rotación base lenta (acumulativa) + parallax de mouse (suavizado con lerp).
      // Separadas para que el parallax no "arrastre" la rotación permanente.
      const base = baseRotationRef.current;
      base.y += delta * 0.05;
      base.x += delta * 0.02;

      const parallax = parallaxRef.current;
      const targetX = pointerRef.current.y * 0.06; // inclinación vertical sutil
      const targetY = pointerRef.current.x * 0.1; // giro horizontal sutil
      // lerp exponencial framerate-independent (~respuesta en ~0.5s)
...
/**
 * SpaceScene — el Canvas 3D real (stars + constelación reactiva al mouse).
 * NO importar directamente: usar SpaceBackground, que lo carga lazy con
 * next/dynamic y decide el fallback estático (reduced-motion / móvil).
 */
  ...
  return (
    <div className="fixed inset-0 z-0">
      <Canvas
        ...
        dpr={[1, 1.75]} // cap de pixel ratio: en pantallas 4K/retina ahorra ~30% de fill-rate sin pérdida visible
      >
